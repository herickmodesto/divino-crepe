import { money } from "../utils/helpers";

export default function FavCard({ item, onAdd, onFavorite }) {
  const isMissing = item.price == null;
  const uid = `fav-heart-${item.id}`;

  const handleRemove = () => {
    onFavorite(item.id, false);
  };

  return (
    <div className="fav-card">

      {/* Imagem + Preço */}
      <div className="fav-card-img-wrap">
        {item.image ? (
          <img src={item.image} alt={item.name} className="fav-card-img" />
        ) : (
          <div className="fav-card-emoji-bg">{item.emoji || "🍽️"}</div>
        )}
        <div className="fav-card-price-tag">
          {isMissing ? "—" : money(item.price)}
        </div>
      </div>

      {/* Coração interno (remover favorito) */}
      <label className="fav-card-heart-label">
        <input
          type="checkbox"
          defaultChecked
          onChange={handleRemove}
          id={uid}
          style={{ position: "absolute", opacity: 0, width: 0, height: 0 }}
        />
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path d="M12 20a1 1 0 0 1-.437-.1C11.214 19.73 3 15.671 3 9a5 5 0 0 1 8.535-3.536l.465.465.465-.465A5 5 0 0 1 21 9c0 6.646-8.212 10.728-8.562 10.9A1 1 0 0 1 12 20z"/>
        </svg>
      </label>

      {/* Conteúdo */}
      <div className="fav-card-body">
        <div className="fav-card-category">{item.category}</div>
        <div className="fav-card-name">{item.name}</div>
        {item.desc && <div className="fav-card-desc">{item.desc}</div>}
        <div className="fav-card-rating">
          {"★★★★☆".split("").map((s, i) => (
            <span key={i} className={s === "★" ? "fav-star on" : "fav-star"}>{s}</span>
          ))}
          <span className="fav-rating-num">(4.8)</span>
        </div>
      </div>

      {/* Botões */}
      <div className="fav-card-actions">
        <button className="fav-card-buy" onClick={() => onAdd(item)}>
          Pedir Agora
        </button>
        <button className="fav-card-cart" onClick={() => onAdd(item)} title="Adicionar ao carrinho">
          <svg viewBox="0 0 27.97 25.074" xmlns="http://www.w3.org/2000/svg">
            <path d="M0,1.175A1.173,1.173,0,0,1,1.175,0H3.4A2.743,2.743,0,0,1,5.882,1.567H26.01A1.958,1.958,0,0,1,27.9,4.035l-2.008,7.459a3.532,3.532,0,0,1-3.4,2.61H8.36l.264,1.4a1.18,1.18,0,0,0,1.156.955H23.9a1.175,1.175,0,0,1,0,2.351H9.78a3.522,3.522,0,0,1-3.462-2.865L3.791,2.669A.39.39,0,0,0,3.4,2.351H1.175A1.173,1.173,0,0,1,0,1.175ZM6.269,22.724a2.351,2.351,0,1,1,2.351,2.351A2.351,2.351,0,0,1,6.269,22.724Zm16.455-2.351a2.351,2.351,0,1,1-2.351,2.351A2.351,2.351,0,0,1,22.724,20.373Z"/>
          </svg>
        </button>
      </div>

    </div>
  );
}
